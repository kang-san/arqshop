...
function MCarousel({
  ...
  const addSlide = (n) => {
    if (slide + n >= cdata.length) setSlide(0);
    else if (slide + n < 0) setSlide(cdata.length - 1);
      ...
      const interval = setInterval(
        () => {
          if (!isPaused) {
            setSlide(index);
            index++;
            if (index >= cdata.length) index = 0;
            if (index < 0) index = cdata.length - 1;
          }
        },
        time ? time : 2000
      );
